...
"""Unit test of pose estimation using BlazePose."""
...
from ml.blazepose import BlazePose
import numpy as np
import pandas as pd
...
class PosenetTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    # Initialize model
    self.blazepose = BlazePose(_MODEL)
  ...
  def test_pose_estimation_image1(self):
    """Test if BP detection's close to ground truth of image1."""
    self._detect_and_assert(self.posenet, self.image_1, self.keypoints_truth_1)

  def test_pose_estimation_image2(self):
    """Test if BP detection's close to ground truth of image2."""
